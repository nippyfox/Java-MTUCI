...
public class FractalExplorer {
    ...
    public void createAndShowGUI() {
        ...
        panel2.add(button, java.awt.BorderLayout.CENTER);
        panel2.add(saveButton, java.awt.BorderLayout.CENTER);
            ...
            if (e.getActionCommand().equals("Сброс")) {
                ...
                if (t == JFileChooser.APPROVE_OPTION) {
                    try {
                        ...
                        JOptionPane.showMessageDialog(image, e1.getMessage(), "Ошибка при сохранении файла", JOptionPane.ERROR_MESSAGE);
                    }
                }
            }
            else {
    ...
    private class FractalWorker extends SwingWorker<Object, Object>{
        ...
        public Object doInBackground() {
            rgb = new int[size];
            for (int i = 0; i < size; i++) {
                int count = fractal.numIterations(FractalGenerator.getCoord(range.x, range.x + range.width, size, i), FractalGenerator.getCoord(range.y, range.y + range.width, size, yCoord));
                if (count == -1) {
                    rgb[i] = 0;
                }
                else {
